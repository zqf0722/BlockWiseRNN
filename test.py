...
parser.add_argument('--front', type=int, default=21,
...
while count <= 6:
    ...
    with torch.no_grad():
        ...
        print('valid Loss{:5.2f}, Perplexity:{:8.2f},best valid Loss{:5.2f},best Perplexity:{:8.2f}' .format(loss_overall,np.exp(loss_overall),best_loss,np.exp(best_loss)))
        print(count)
    epoch += 1  
    j += 1  
#测试
...
with torch.no_grad():
        ...
        for i in range(0, test_data.size(1) - args.bptt, args.bptt):
            inputs3 = test_data[:,i:i+args.bptt].to(device)
            targets3 = test_data[:,(i+1):(i+1)+args.bptt].to(device) # 输出相对输入错一位，往后顺延一个单词
            h_test = detach(h_test)
            outputs3,h_test = model(inputs3,h_test)
 
            # 计算perplexity
            
            loss3 = criterion(outputs3,targets3.reshape(-1))
            loss_overall=loss3.item()+loss_overall
        loss_overall=loss_overall/num_batches2
